  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-text-primary tracking-tight">Anomaly Detection</h1>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-bg-card border border-border/60 rounded-xl shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-semibold text-text-secondary">SPC Control Chart — Car Deck Position</h3>
      ...
      <div className="bg-bg-card border border-border/60 rounded-xl shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-border flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-text-secondary">Flagged Anomalous Runs ({flagged.length})</h3>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-bg-card border-b border-border shadow-[0_1px_3px_rgba(0,0,0,0.3)]">
              <tr className="text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                ...
                <th className="px-4 py-3 text-right">Car Deck</th>
                <th className="px-4 py-3 text-right pr-6">Rate</th>
                <th className="px-4 py-3 text-left pl-6">Deviations</th>
              </tr>
            </thead>
            <tbody>
              {flagged.map((a: BakeAnomaly, idx: number) => (
                <tr key={a.run_number} className={`border-b border-border/50 hover:bg-white/[0.03] ${idx % 2 === 1 ? 'bg-white/[0.01]' : ''}`}>
                  <td className="px-4 py-3 font-mono text-text-primary">{a.run_number}</td>
                  <td className="px-4 py-3 text-text-secondary">{a.furnace}</td>
                  <td className="px-4 py-3 text-center">{a.severity && <StatusBadge status={a.severity} />}</td>
                  <td className="px-4 py-3 text-right font-mono">{a.car_deck}</td>
                  <td className="px-4 py-3 text-right"><DefectRateBadge rate={a.defect_rate} /></td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1.5">
                      {a.deviations.map((d: Deviation, i: number) => (
                        <span key={i} className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-bg-input text-text-secondary">
...
function GraphiteRisk() {
  ...
  return (
    <div className="space-y-8">
      {/* Quintile reference */}
      <div className="bg-bg-card border border-border/60 rounded-xl shadow-sm p-6">
        <h3 className="text-base font-semibold text-text-secondary mb-4">Composition Risk Quintiles</h3>
        <div className="grid grid-cols-5 gap-3">
          {quintileCounts.map((q) => (
            <div key={q.quintile}
              ...
              <div className="text-xl font-bold font-mono" style={{ color: QUINTILE_COLORS[q.quintile] }}>{q.quintile}</div>
              <div className="text-xs text-text-muted mt-1.5">Avg Rate</div>
              <div className="font-mono text-sm text-text-primary">{(q.avg_defect_rate * 100).toFixed(1)}%</div>
              <div className="text-xs text-text-muted mt-1.5">P(High Event)</div>
              <div className="font-mono text-sm text-text-primary">{(q.probability_high_defect_event * 100).toFixed(0)}%</div>
              <div className="text-xs text-text-muted mt-2">{q.count} runs</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-bg-card border border-border/60 rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-border">
            <h3 className="text-base font-semibold text-text-secondary">Graphite Runs — Risk Assessment</h3>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-bg-card border-b border-border shadow-[0_1px_3px_rgba(0,0,0,0.3)]">
                <tr className="text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3 text-right">Hi-Risk Lots</th>
                </tr>
              </thead>
              <tbody>
                {assessments.map((a: GraphiteAssessment, idx: number) => (
                  <tr key={a.run_number} onClick={() => setSelectedRun(a.run_number)}
                    className={`border-b border-border/50 cursor-pointer transition-colors hover:bg-white/[0.03] ${
                      selectedRun === a.run_number ? 'bg-accent-glow' : idx % 2 === 1 ? 'bg-white/[0.01]' : ''
                    }`}>
                    <td className="px-4 py-3 font-mono text-text-primary">{a.run_number}</td>
                    <td className="px-4 py-3 text-text-secondary">{a.furnace}</td>
                    <td className="px-4 py-3 text-center"><StatusBadge status={a.risk_score || 'no_data'} /></td>
                    <td className="px-4 py-3 text-right"><DefectRateBadge rate={a.defect_rate} /></td>
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3 mb-5 text-sm">
                <div className="bg-bg-input rounded-lg p-3">
                  <div className="text-xs text-text-muted">High-Risk Lots</div>
                  <div className="font-mono text-lg text-text-primary mt-1">{selected.composition?.high_risk_lot_count}</div>
                </div>
                <div className="bg-bg-input rounded-lg p-3">
                  <div className="text-xs text-text-muted">Edge Positions</div>
                  <div className="font-mono text-lg text-text-primary mt-1">{selected.composition?.edge_position_count}</div>
                </div>
                <div className="bg-bg-input rounded-lg p-3">
                  <div className="text-xs text-text-muted">Avg Lot Risk</div>
                  <div className="font-mono text-lg text-text-primary mt-1">{((selected.composition?.avg_lot_defect_rate || 0) * 100).toFixed(1)}%</div>
                </div>
              </div>
              {/* Electrode list */}
              <div className="text-sm space-y-1 max-h-[300px] overflow-y-auto">
                <div className="flex text-xs text-text-muted uppercase tracking-wider px-3 py-2">
                ...
                {selected.composition?.electrodes?.map((e: CompositionElectrode) => (
                  <div key={e.gpn} className={`flex items-center px-3 py-1.5 rounded ${
                    e.defect_code_og || e.defect_code_of ? 'bg-danger-dim/30' : ''
                  }`}>
                    <span className="w-10 font-mono">{e.position_og}</span>
                    <span className="flex-1 text-text-secondary">
                      {e.lot}
                      {e.risk_tier === 'high' && <span className="ml-1 text-danger">●</span>}
                    </span>
                    <span className="w-20 text-right font-mono">{((e.lot_defect_rate || 0) * 100).toFixed(1)}%</span>
